function [signals_pulse, signals_found, signals_lost] = Drawer(folder, input_filename, len_line, grid_hz)
signal = ReadFile([folder, input_filename], 'int32', 1);
raw = reshape(signal, [len_line length(signal) / len_line])';
signals_pulse = LoadFound([folder 'finiteSignal.bin']);
...
for i = 1:length(signals_pulse)
    if(signals_pulse(i).meanSnr > 0)
        signals_pulse(i).startFreq = (signals_pulse(i).startFreq-0.5 - len_line / 2) * grid_hz / 1e6;
        signals_pulse(i).stopFreq = (signals_pulse(i).stopFreq+0.5 - len_line / 2) * grid_hz / 1e6;
        rectangle('Position', ...
